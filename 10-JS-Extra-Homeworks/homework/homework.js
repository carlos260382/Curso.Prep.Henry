...
for (var i=0; i<s.length; i++){
  if (s[i]===s[i].toUpperCase()){
    texMayuscula = texMayuscula + s[i];
  } else
   {
texMinuscula = texMinuscula + s[i];
  } 
 }
 return texMayuscula + texMinuscula;
}
...
function capicua(numero) {
  ...
  for (i = numeroString.length - 1; i >= 0; i--) {
    numeroCapi = numeroCapi + numeroString[i];
  }
  if (numeroCapi !== numeroString) {
